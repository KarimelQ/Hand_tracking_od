...
std::vector<float> HandDetector::Inference() {
    cv::Point pt;
    cv::Mat image = cv::imread("test_image.jpg");

    std::vector<float> result;

    pt = findMidpoint(image);

    result.push_back(static_cast<float>(pt.x));
    result.push_back(static_cast<float>(pt.y));
    return result;
}

namespace py = pybind11;

PYBIND11_MODULE(hand_detector_wrapped, m) {
    py::class_<HandDetector>(m, "HandDetector")
        .def(py::init<const char *>(), "Constructor for HandDetector")
        .def("findMidpoint", &HandDetector::findMidpoint)
        .def("drawBoundingBox" , &HandDetector::drawBoundingBox)
        .def("getBoundingBox"  , &HandDetector::getBoundingBox)
        .def("Inference", &HandDetector::Inference);

    m.def("Output_to_points", Output_to_points, "Function to convert output to points");
    // m.def("process_image", &process_image, "Process an image using OpenCV");
}